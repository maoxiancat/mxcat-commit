...
脚本路径相对于本技能目录（含 `SKILL.md` 的那一层），不是目标仓库根目录。默认整棵工作树时带 `--add`。文件只暂存了一截、工作区还有更多改动时，这条 commit 只含 index 里已有的内容，未暂存部分留下。index 与 HEAD 相同、或路径尚未进入 index 时，`--add` 把工作区全文纳入这一条。已暂存的 rename 可以 `--add` 旧路径和新路径；只给旧路径会在提交前退出。已暂存的删除可以 `--add`。删除后又出现的文件不进入 commit。copy 的旧路径按普通文件处理。同一路径的两种写法只换入一次。用户只要暂存区时不要带 `--add`，也不要自己 `git add`。这些路径上若还有未暂存改动，脚本提交 index 里已有的内容并留下工作区；index 与 HEAD 相同则在提交前退出。
