...
脚本路径相对于本技能目录（含 `SKILL.md` 的那一层），不是目标仓库根目录。默认整棵工作树时带 `--add`，同一文件的已暂存和未暂存改动一起进入这一条。用户只要暂存区时不要带 `--add`，也不要自己 `git add`。这些路径上若还有未暂存改动，脚本提交 index 里已有的内容并留下工作区；index 与 HEAD 相同则在提交前退出。
