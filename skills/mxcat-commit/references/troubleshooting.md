...
同一路径用两种写法传入（例如 `./f.txt` 与 `f.txt`）时，未暂存内容仍留在工作区，不会在恢复时被盖掉。带 `--add` 时也一样。`--hunks` 与另一个已暂存文件一起提交、且不带 `--add` 时，那个文件的 commit 只含 index，未暂存内容不得进入历史，也不得从工作区消失。当前目录不是仓库根时，按 hunk 或只提交已暂存内容应与在仓库根调用时相同，不得在提交前被拒绝。index 与 HEAD 相同但工作区仍有未暂存改动时，仍在创建 commit 之前退出。

已暂存的删除可以 `--add`。删除已暂存但文件又出现在工作区时，不论是否带 `--add`，提交的都是这次删除，后出现的内容留在工作区；创建 commit 之前失败时，该文件会放回。改名不再被识别为 rename、而是删除加新增时，`--add` 仍可同时提交旧路径和新路径。只给仍被识别为 rename 的旧路径时，在创建 commit 之前退出，index 保持这次改名。copy 的旧路径不按 rename 跳过：有未暂存改动时只提交 index。
