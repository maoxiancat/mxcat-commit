...
同一路径用两种写法传入（例如 `./f.txt` 与 `f.txt`）时，未暂存内容仍留在工作区，不会在恢复时被盖掉。`--hunks` 与另一个已暂存文件一起提交、且不带 `--add` 时，那个文件的 commit 只含 index，未暂存内容不得进入历史，也不得从工作区消失。当前目录不是仓库根时，按 hunk 或只提交已暂存内容应与在仓库根调用时相同，不得在提交前被拒绝。index 与 HEAD 相同但工作区仍有未暂存改动时，仍在创建 commit 之前退出。
